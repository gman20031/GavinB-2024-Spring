...
World::World(SDL_Renderer* pRenderer)
	: m_ppGrid(nullptr)
	, m_width(0)
	, m_height(0)
	, m_length(0)
...
void World::CreatePlayer(int x, int y)
{
	...
	m_pPlayer = ActorFactory::Createplayer(this, { x,y }, kPlayerStartingHealth , m_pRenderer);
...
void World::GenerateEnemies(size_t amount)
{
	for (size_t i = 0; i < amount; ++i)
	{
		...
		switch (rand() % 2)
		{
		case 0: newEnemy = ActorFactory::CreateScaredEnemy(this, { x,y } , m_pRenderer); break;
		case 1: newEnemy = ActorFactory::CreateDirectEnemy(this, { x,y } , m_pRenderer); break;
		}
...
Actor* World::CreateTwoTeleporters(int x, int y)
{
	...
	auto randomizedTeleporter = ActorFactory::CreateTeleporterTile(this, { randomX,randomY }, m_pRenderer);
	auto returnedTeleporter = ActorFactory::CreateTeleporterTile(this, { x,y } , m_pRenderer);
...
Actor* World::CreateActorFromType(ActorFactory::FactoryPrototype type, int x, int y)
{
	...
	switch (type)
	{
	case kEmptyTile:		return ActorFactory::CreateEmptyTile(this, { x,y } , m_pRenderer);
	case kBombTile:			return ActorFactory::CreateBombTile(this, { x,y }, m_pRenderer);
	case kTreasureTile:		return ActorFactory::CreateTreasureTile(this, { x,y }, m_pRenderer);
	case kMimicTile:		return ActorFactory::CreateMimicTile(this, { x,y }, m_pRenderer);
...
void World::GenerateWorld()
{
	...
	{
		Actor* temp = ActorFactory::CreateEmptyTile(this, position , m_pRenderer);
		m_ppGrid[Position_tToIndex(position)] = temp;
	}

	// creates a tile at start and end
	m_ppGrid[0] = ActorFactory::CreateEmptyTile(this, { 0,0 } , m_pRenderer);
	m_ppGrid[m_length - 1] = ActorFactory::CreateExitTile(this, { m_width - 1,m_height - 1 } , m_pRenderer);
