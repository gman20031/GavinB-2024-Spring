...
LevelEditor::LevelEditor()
	...
	, m_chosenFile()
{
	m_currentFilePath = m_defaultFolderPath;
	InitButtons();
	InitDefaultMap();
}
...
void LevelEditor::Save()
{
	...
	std::string fullFilePath = m_defaultFolderPath + m_chosenFile;
...
void LevelEditor::SaveAs()
{
	...
	for (const auto& entry : std::filesystem::directory_iterator(m_defaultFolderPath))
	{
	...
	if (fileExists)
	{
		if (PromptAndGetChar("File already exists, overrite file?", { 'y','n' }) == 'n')
		{
			return;
		}
	}
...
void LevelEditor::Load()
{
	...
	m_chosenFile = ChooseFromFileDirectory(m_defaultFolderPath);

	DeleteLevelArray();
	auto MapArray = MapFileLoader::ConvertToCString(m_currentFilePath + m_chosenFile);
	ChangeArraySize(MapArray.m_width, MapArray.m_length);
